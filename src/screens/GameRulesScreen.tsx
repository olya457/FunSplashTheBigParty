...
const TITLE_IMG: ImageSourcePropType = require('../assets/title.png');
...
export default function GameRulesScreen({ navigation }: Props) {
  ...
  const bubbles = useMemo<Bubble[]>(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      arr.push({ key: `rb${i}`, size, startX, drift, delay, duration, progress: new Animated.Value(0) });
    }
    return arr;
  }, [width]);
  ...
  return (
    <View style={styles.wrap}>
      <ImageBackground source={BG} style={styles.bg} resizeMode="cover">
        {renderBubbles()}
        <Animated.View
          style={[styles.container, { paddingTop: 80 * s, opacity: fade, transform: [{ translateY: slide }] }]}
        >
          ...
          >
            <ScrollView
              ...
              contentContainerStyle={{ padding: 14 * s }}
              keyboardShouldPersistTaps="handled"
            >
              <Text style={[styles.ruleText, { fontSize: Math.max(13, 14 * s), lineHeight: Math.max(18, 20 * s) }]}>
{`Player Turn:
All players take turns casting a fishing rod.
With each cast, an object is randomly caught.

Catches:
Ordinary objects (fish, circle, box, etc.) bring a task or challenge that the player must complete.
Tasks can be funny, active, or creative — they add fun to the game.

Goldfish:
If a goldfish falls out, the game ends immediately.
The player who caught the goldfish becomes the winner.
The winner makes a wish that all other players must fulfill.

End of the game:
The game ends when a goldfish falls out.
If the goldfish has not yet appeared — the game continues until someone catches it.`}
              </Text>
            </ScrollView>
          </View>
        </Animated.View>
